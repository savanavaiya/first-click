@if (isset($rightdata))
    <div id="rightsidebox">
        <div>
            ...
            <div class="row headingbox">
                                ...
                                <div class="bordcuss">
                                    <div class="modal-body p-0">
                                        ...
                                        <div class="d-flex justify-content-center mb-5">
                                            <span class="alerttx">Are you sure you want to Reject this Approval
                                                Data?</span>
                                        </div>
                ...
                <div class="col d-flex justify-content-end align-items-center">
                    <button class="editbox2" id="approve">
                        <span class="edittx2">Approve</span>
                    </button>
                </div>
            </div>
        ...
        <div class="headingbox">
            <div>
                <span class="storenametx">Add station request by</span>
            </div>
            <div class="my-1">
                <span class="name2">{{ $rightdata->nick_name }}</span>
            </div>
        </div>
        ...
        <div class="headingbox">
            ...
            <div class="my-1">
                ...
                <span id="city_errorafterapprove" class="errortx"></span>
            </div>
        </div>
        ...
        <div class="headingbox">
            <div>
                <span class="otherinfo">Feedback/Comments</span>
            </div>
            <div class="my-1">
                <span class="otherinfotx">{{ $rightdata->comments == null ? '-' : $rightdata->comments }}</span>
            </div>
        </div>

        <div class="headingbox">
            <div>
                <span class="otherinfo">Date of submission</span>
            </div>
            <div class="my-1">
                <span class="otherinfotx dateofsub"></span>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="my-2">
                    <div class="store_imagebox">
                        @if ($rightdata->store_image == null)
                            <img src="{{ asset('assets/images/imagenotavailable.png') }}" alt="store_image"
                                class="store_image">
                        @else
                            <a href="{{ asset('public/images/storeimage/' . $rightdata->store_image) }}"
                                target="blank"><img
                                    src="{{ asset('public/images/storeimage/' . $rightdata->store_image) }}"
                    ...
                    @if ($rightdata->status == '1')
                        <div class="row">
                            <div class="col-md-6">
                                <span class="storeimage">Brand Logo</span>
                            </div>
                        </div>
                        <div class="my-2">
                            <div class="store_imagebox">
                                @if ($rightdata->brand_logo == null)
                                    <img src="{{ asset('assets/images/imagenotavailable.png') }}" alt="store_image"
                                        class="store_image">
                                @else
                                    <a href="{{ asset('public/images/brandlogo/' . $rightdata->brand_logo) }}"
                                        target="blank"><img
                                            src="{{ asset('public/images/brandlogo/' . $rightdata->brand_logo) }}"
                                            alt="store_image" class="store_image"></a>
                                @endif
                            </div>
                        </div>
        ...
        <form id="formupdatedata">
            ...
            <div class="row headingbox">
                <div class="col-md-10 d-flex justify-content-start align-items-center">
                    <span class="rightbxeditheade">Edit Station For Approval Details</span>
                </div>
            ...
            <div class="headingbox">
                ...
                <div class="my-1">
                    {{-- <span class="name2">{{ $rightdata->nick_name }}</span> --}}
                    <input type="text" name="nick_name" id="" class="fld2"
                        value="{{ $rightdata->nick_name }}">
                    <span id="nick_name_errorupd" class="errortx errornull"></span>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="headingbox">
                        ...
                        <div class="my-1">
                            {{-- <span class="otherinfotx">{{ $rightdata->comments }}</span> --}}
                            <input type="text" name="comments" id="" class="fld2"
                                value="{{ $rightdata->comments }}">
    ...
    <script>
        $('#rejectconfirm').click(function() {
            ...
            Loader();
            $.ajax({
                url: "{{ route('rejectforapprovastation') }}",
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id
                ...
                success: function(data) {
                    ...
                    if (data == true) {
                        swal({
                            title: "Station has been rejected.",
                            icon: "success",
                        }).then(function() {
                            localStorage.setItem("forapproval", '1');
                            location.reload();
                        });
                    }
                    if (data == false) {
    ...
    <script>
        $('#approve').click(function() {
            ...
            Loader();
            $.ajax({
                url: "{{ route('approveforapprovastation') }}",
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id
                ...
                success: function(data) {
                    ...
                    if (data.data) {
                        swal({
                            title: "Station has been approved.",
                            icon: "success",
                        }).then(function() {
                            localStorage.setItem("afterupdateid", data.data.id);
                            location.reload();
                        });
                    }
                    if (data == false) {
                        // $('#city_errorafterapprove').html('Please enter city name');
    ...
    <script>
        ...
        $("form#formupdatedata").submit(function(e) {
            ...
            Loader();
            $.ajax({
                url: "{{ route('updateforapprovalstationdata') }}",
                type: 'POST',
                data: formData,
                contentType: false,
                cache: false,
    ...
    <script>
        $(document).ready(function() {
            // UTC time string
            const utc = new Date(<?php echo strtotime($rightdata->created_at) * 1000; ?>);
            ...
            $('.dateofsub').html( localMoment.format(
